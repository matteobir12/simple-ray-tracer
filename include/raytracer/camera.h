...
namespace RayTracer
{
    ...
    public:
        Camera(CameraSettings settings)
            : cameraSettings(settings), width(0), height(0), pixelSamplesScale(0.0f), center(), pixel00Loc(), pixelDeltaU(), pixelDeltaV(), u(), v(), w(), defocusDiskU(), defocusDiskV(), position(settings.origin), front(glm::normalize(settings.lookAt - settings.origin)), up(settings.vUp), right(glm::normalize(glm::cross(front, up))), yaw(-90.0f), pitch(0.0f)
        {
        }
